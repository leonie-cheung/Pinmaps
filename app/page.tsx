...
export default function Page() {
  ...
  const sortedCategories = useMemo(() => {
    return Object.keys(postsByCategory).sort(
        (a, b) =>
            (postsByCategory[b]?.length ?? 0) - (postsByCategory[a]?.length ?? 0)
    );
  }, [postsByCategory]);
  ...
  return (
      <div className="min-h-screen bg-white text-zinc-900 font-sans selection:bg-pink-100">
        ...
        {/* Navigation Header (your top bar) */}
          ...
          {error && (
              <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                ...
                <p className="mt-2 text-red-600">
                  Common causes: RLS still ON, wrong Supabase keys, or you didn’t
          ...
          <div className="flex overflow-x-auto gap-8 pb-12 no-scrollbar">
            ...
            {sortedCategories.map((category) => {
              ...
              return (
                  <div
                      key={category}
                      className="flex-shrink-0 w-64 md:w-72 flex flex-col gap-4"
                  >
                    {/* Category Title */}
                  ...
                  <span className="text-sm font-medium text-zinc-500 uppercase tracking-wider">
                    ...
                    {/* Show up to 2 posts per category */}
                    {posts.slice(0, 2).map((post) => (
                        <PostCard key={post.id} post={post} colourClass={colour} />
                    ))}

                    {/* If a category has only 1 post, show a placeholder */}
                    {posts.length === 1 ? (
                        <PlaceholderCard colourClass={colour} />
                    ) : null}
                  </div>
              );
            })}
      ...
      >
        ...
        {(post.caption || post.rating != null) && (
            <div className="absolute bottom-0 left-0 right-0 bg-white/80 backdrop-blur p-3">
              {post.caption && (
                  <p className="text-sm font-medium text-zinc-900">{post.caption}</p>
              )}
              {post.rating != null && (
                  <p className="text-xs text-zinc-600 mt-1">{post.rating}/5</p>
              )}
            </div>
        )}
      </div>
